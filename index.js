...
client.on("voiceStateUpdate", async v => {
  if(v.streaming == true) (await client.channels.fetch("1023184564039991337")).send(`${(await client.users.fetch(v.id)).tag} started streaming in <#${v.channelId}>`)
})

client.on("guildMemberUpdate", async(o, n) => {
  if(o.nickname != n.nickname) {
    (await client.channels.fetch("1023184564039991337")).send(`${n.user.tag} changed their nickname from \`${o.nickname}\` to \`${n.nickname}\`.`)
  }
  ...
  if(o.roles.cache.size != n.roles.cache.size) {
    ...
    o.roles.cache.forEach(role => {
      ...
      (await client.channels.fetch("1023184564039991337")).send(`${n.user.tag} roles got updated!\nAdded roles: ${ar.length != 0 ? ar.join(" ") : "No roles added"}\nRemoved roles: ${ar2.length != 0 ? ar2.join(" ") : "No roles removed"}`)
        })
    
  }
})
client.on("guildMemberAdd", async m => {
  (await client.channels.fetch("1023184564039991337")).send(`${m.user.tag} joined the server!`)
})
client.on("guildMemberRemove",async m => {
  (await client.channels.fetch("1023184564039991337")).send(`${m.user.tag} left the server :(`)
})
client.on("messageDelete",async m => {
  (await client.channels.fetch("1023184564039991337")).send(`${m.author.tag}'s message got deleted in ${m.channel}! Content: ${m.content}`)
})
// --- Startup ---------------------------------------------------------------------------------------------------------
